...
const allergyOptions = ['Egg', 'Peanut', 'Dairy', 'Wheat', 'Sesame', 'Soy'];
...
for (const funnel of funnels) {
  test.describe(`${funnel}: Allergy Selector Validation`, () => {
    test('can select multiple allergy options', async ({ page }) => {
      await page.goto(getTestingUrl(`${funnel}/food-allergies`));
      await page.waitForURL(new RegExp(`/${funnel}/food-allergies`), { timeout: 10_000 });

      await clickOption(page, 'Egg');
      await clickOption(page, 'Peanut');
      await clickOption(page, 'Dairy');

      expect(await isOptionSelected(page, 'Egg')).toBe(true);
      expect(await isOptionSelected(page, 'Peanut')).toBe(true);
      expect(await isOptionSelected(page, 'Dairy')).toBe(true);
    });
    ...
    test('selecting No deselects all other options', async ({ page }) => {
      ...
      await clickOption(page, 'Egg');
      await clickOption(page, 'Dairy');
      await clickOption(page, 'Wheat');

      expect(await isOptionSelected(page, 'Egg')).toBe(true);
      expect(await isOptionSelected(page, 'Dairy')).toBe(true);
      expect(await isOptionSelected(page, 'Wheat')).toBe(true);
    ...
    test('tapping an option toggles its selection state', async ({ page }) => {
      ...
      await clickOption(page, 'Dairy');
      expect(await isOptionSelected(page, 'Dairy')).toBe(false);
